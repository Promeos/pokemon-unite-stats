...
opponent deleting un-invested me"). Metric: basic-attack hits-to-kill and
seconds-to-kill. Basic-attacks only (ability/boosted-attack burst is additive and
...
from builds import hits_between, load_data, tier_build, ttk_between
...
def investment_table(data: dict) -> pd.DataFrame:
    ...
    for mon in OFFENSIVE:
        for lvl in PRE_EVO_LEVELS:
            ...
            t0, t1 = ttk_between(bare, target), ttk_between(maxed, target)
            rows.append(dict(
                pokemon=mon, level=lvl,
                hits_uninvested=h0, hits_maxed=h1, hits_saved=h0 - h1,
                hits_reduction_pct=round(100 * (h0 - h1) / h0, 1),
                ttk_uninvested=round(t0, 2), ttk_maxed=round(t1, 2),
                ttk_reduction_pct=round(100 * (t0 - t1) / t0, 1) if t0 else 0.0,
            ))
...
def plot_hits(df: pd.DataFrame) -> str:
    ...
    for ax, mon in zip(axes, OFFENSIVE):
        ...
        ax.set_xlabel("Level")
        ax.grid(alpha=0.3)
    axes[0].set_ylabel("Basic attacks to kill a squishy")
...
def main():
    ...
    print()
    lvl3 = df[df.level == 3]
    print("=== Headline @ Lv3 (pre-evo) - hits to delete an un-invested squishy ===")
    for _, r in lvl3.iterrows():
        print(f"  {r.pokemon.capitalize():10s}: {int(r.hits_uninvested):2d} -> {int(r.hits_maxed):2d} hits "
              f"({r.hits_reduction_pct:.0f}% fewer),  {r.ttk_uninvested:.1f}s -> {r.ttk_maxed:.1f}s")
    print(f"\nMean hits-reduction across Lv1-6, all mons: {df.hits_reduction_pct.mean():.1f}%")
    print(f"Mean TTK-reduction  across Lv1-6, all mons: {df.ttk_reduction_pct.mean():.1f}%")
    print(f"\nSaved: {out_csv}\n       {fig_path}")
