...
_GRADE_NAME = {"A": "Gold", "B": "Silver", "C": "Bronze"}
_STAT_LABEL = {"hp": "HP", "attack": "Atk", "defense": "Def", "sp_attack": "Sp.Atk",
               "sp_defense": "Sp.Def", "crit": "Crit", "cdr": "CDR", "speed": "Move"}


def _stats_str(e):
    return ", ".join(f"{_STAT_LABEL.get(k, k)} {'+' if v > 0 else ''}{v}" for k, v in _estats(e).items())


def pages_report(rarity="gold"):
    """The exact named emblems chosen for each template, as a printable string."""
    lines = [f"Optimal {rarity} emblem pages — the exact named emblems chosen per template",
             "(generated by emblems.py from unite-db/emblems.json)\n"]
    for tmpl, target in TEMPLATE_TARGET.items():
        page = _select_page(target, rarity)
        flat, pct = optimal_page(target, rarity)
        nz = {k: round(v, 1) for k, v in flat.as_dict().items() if round(v, 1)}
        pz = {k: v for k, v in pct.as_dict().items() if v}
        lines.append(f"=== {tmpl}  ({page_summary(tmpl, rarity)}) ===")
        for e in page:
            lines.append(f"  {_GRADE_NAME[e['grade']]} {e['display_name']:13} "
                         f"[{'/'.join(colors_of(e))}]   {_stats_str(e)}")
        lines.append(f"  -> net flat: {nz}   color-set bonus: {pz}\n")
    return "\n".join(lines)


if __name__ == "__main__":
    report = pages_report("gold")
    print(report)
    with open(os.path.join(DATA_DIR, "emblem_pages.txt"), "w", encoding="utf-8") as fh:
        fh.write(report + "\n")
    print(f"wrote {os.path.join(DATA_DIR, 'emblem_pages.txt')}")
